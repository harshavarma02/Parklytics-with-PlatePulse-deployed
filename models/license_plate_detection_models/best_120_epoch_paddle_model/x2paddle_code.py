...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.conv2d0 = paddle.nn.Conv2D(
            stride=2, padding=1, out_channels=16, kernel_size=(3, 3), in_channels=3
        )
        self.silu0 = paddle.nn.Silu()
        self.conv2d1 = paddle.nn.Conv2D(
            stride=2, padding=1, out_channels=32, kernel_size=(3, 3), in_channels=16
        )
        self.silu1 = paddle.nn.Silu()
        self.conv2d2 = paddle.nn.Conv2D(
            out_channels=32, kernel_size=(1, 1), in_channels=32
        )
        self.silu2 = paddle.nn.Silu()
        self.conv2d3 = paddle.nn.Conv2D(
            padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16
        )
        self.silu3 = paddle.nn.Silu()
        self.conv2d4 = paddle.nn.Conv2D(
            padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16
        )
        self.silu4 = paddle.nn.Silu()
        self.conv2d5 = paddle.nn.Conv2D(
            out_channels=32, kernel_size=(1, 1), in_channels=48
        )
        self.silu5 = paddle.nn.Silu()
        self.conv2d6 = paddle.nn.Conv2D(
            stride=2, padding=1, out_channels=64, kernel_size=(3, 3), in_channels=32
        )
        self.silu6 = paddle.nn.Silu()
        self.conv2d7 = paddle.nn.Conv2D(
            out_channels=64, kernel_size=(1, 1), in_channels=64
        )
        self.silu7 = paddle.nn.Silu()
        self.conv2d8 = paddle.nn.Conv2D(
            padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32
        )
        ...
        self.conv2d10 = paddle.nn.Conv2D(
            padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32
        )
        self.silu10 = paddle.nn.Silu()
        self.conv2d11 = paddle.nn.Conv2D(
            padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32
        )
        self.silu11 = paddle.nn.Silu()
        self.conv2d12 = paddle.nn.Conv2D(
            out_channels=64, kernel_size=(1, 1), in_channels=128
        )
        self.silu12 = paddle.nn.Silu()
        self.conv2d13 = paddle.nn.Conv2D(
            stride=2, padding=1, out_channels=128, kernel_size=(3, 3), in_channels=64
        )
        self.silu13 = paddle.nn.Silu()
        self.conv2d14 = paddle.nn.Conv2D(
            out_channels=128, kernel_size=(1, 1), in_channels=128
        )
        self.silu14 = paddle.nn.Silu()
        self.conv2d15 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        ...
        self.conv2d17 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu17 = paddle.nn.Silu()
        self.conv2d18 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu18 = paddle.nn.Silu()
        self.conv2d19 = paddle.nn.Conv2D(
            out_channels=128, kernel_size=(1, 1), in_channels=256
        )
        self.silu19 = paddle.nn.Silu()
        self.conv2d20 = paddle.nn.Conv2D(
            stride=2, padding=1, out_channels=256, kernel_size=(3, 3), in_channels=128
        )
        self.silu20 = paddle.nn.Silu()
        self.conv2d21 = paddle.nn.Conv2D(
            out_channels=256, kernel_size=(1, 1), in_channels=256
        )
        self.silu21 = paddle.nn.Silu()
        self.conv2d22 = paddle.nn.Conv2D(
            padding=1, out_channels=128, kernel_size=(3, 3), in_channels=128
        )
        self.silu22 = paddle.nn.Silu()
        self.conv2d23 = paddle.nn.Conv2D(
            padding=1, out_channels=128, kernel_size=(3, 3), in_channels=128
        )
        self.silu23 = paddle.nn.Silu()
        self.conv2d24 = paddle.nn.Conv2D(
            out_channels=256, kernel_size=(1, 1), in_channels=384
        )
        ...
        self.conv2d26 = paddle.nn.Conv2D(
            out_channels=256, kernel_size=(1, 1), in_channels=512
        )
        self.silu26 = paddle.nn.Silu()
        self.conv2d27 = paddle.nn.Conv2D(
            out_channels=128, kernel_size=(1, 1), in_channels=384
        )
        self.silu27 = paddle.nn.Silu()
        self.conv2d28 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        ...
        self.conv2d30 = paddle.nn.Conv2D(
            out_channels=128, kernel_size=(1, 1), in_channels=192
        )
        self.silu30 = paddle.nn.Silu()
        self.conv2d31 = paddle.nn.Conv2D(
            out_channels=64, kernel_size=(1, 1), in_channels=192
        )
        ...
        self.conv2d33 = paddle.nn.Conv2D(
            padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32
        )
        self.silu33 = paddle.nn.Silu()
        self.conv2d34 = paddle.nn.Conv2D(
            out_channels=64, kernel_size=(1, 1), in_channels=96
        )
        self.silu34 = paddle.nn.Silu()
        self.conv2d35 = paddle.nn.Conv2D(
            stride=2, padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu35 = paddle.nn.Silu()
        self.conv2d36 = paddle.nn.Conv2D(
            out_channels=128, kernel_size=(1, 1), in_channels=192
        )
        self.silu36 = paddle.nn.Silu()
        self.conv2d37 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        ...
        self.conv2d39 = paddle.nn.Conv2D(
            out_channels=128, kernel_size=(1, 1), in_channels=192
        )
        self.silu39 = paddle.nn.Silu()
        self.conv2d40 = paddle.nn.Conv2D(
            stride=2, padding=1, out_channels=128, kernel_size=(3, 3), in_channels=128
        )
        ...
        self.conv2d42 = paddle.nn.Conv2D(
            padding=1, out_channels=128, kernel_size=(3, 3), in_channels=128
        )
        self.silu42 = paddle.nn.Silu()
        self.conv2d43 = paddle.nn.Conv2D(
            padding=1, out_channels=128, kernel_size=(3, 3), in_channels=128
        )
        ...
        self.x635 = self.create_parameter(
            dtype="float32",
            shape=(1, 8400),
            default_initializer=paddle.nn.initializer.Constant(value=0.0),
        )
        self.conv2d45 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        ...
        self.conv2d47 = paddle.nn.Conv2D(
            out_channels=64, kernel_size=(1, 1), in_channels=64
        )
        ...
        self.conv2d49 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu48 = paddle.nn.Silu()
        self.conv2d50 = paddle.nn.Conv2D(
            out_channels=1, kernel_size=(1, 1), in_channels=64
        )
        self.conv2d51 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=128
        )
        self.silu49 = paddle.nn.Silu()
        self.conv2d52 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu50 = paddle.nn.Silu()
        self.conv2d53 = paddle.nn.Conv2D(
            out_channels=64, kernel_size=(1, 1), in_channels=64
        )
        self.conv2d54 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=128
        )
        self.silu51 = paddle.nn.Silu()
        self.conv2d55 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu52 = paddle.nn.Silu()
        self.conv2d56 = paddle.nn.Conv2D(
            out_channels=1, kernel_size=(1, 1), in_channels=64
        )
        self.conv2d57 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=256
        )
        self.silu53 = paddle.nn.Silu()
        self.conv2d58 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu54 = paddle.nn.Silu()
        self.conv2d59 = paddle.nn.Conv2D(
            out_channels=64, kernel_size=(1, 1), in_channels=64
        )
        self.conv2d60 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=256
        )
        self.silu55 = paddle.nn.Silu()
        self.conv2d61 = paddle.nn.Conv2D(
            padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64
        )
        self.silu56 = paddle.nn.Silu()
        self.conv2d62 = paddle.nn.Conv2D(
            out_channels=1, kernel_size=(1, 1), in_channels=64
        )
        self.softmax0 = paddle.nn.Softmax(axis=1)
        self.conv2d63 = paddle.nn.Conv2D(
            out_channels=1, kernel_size=(1, 1), bias_attr=False, in_channels=16
        )
        self.x854 = self.create_parameter(
            dtype="float32",
            shape=(1, 2, 8400),
            default_initializer=paddle.nn.initializer.Constant(value=0.0),
        )
    ...
    def forward(self, x0):
        ...
        x390 = [2.0, 2.0]
        x391 = paddle.nn.functional.interpolate(
            x=x388, scale_factor=x390, mode="nearest"
        )
        x393 = [x391, x284]
        ...
        x447 = [2.0, 2.0]
        x448 = paddle.nn.functional.interpolate(
            x=x445, scale_factor=x447, mode="nearest"
        )
        x450 = [x448, x197]
...
def main(x0):
    ...
    params = paddle.load(
        r"C:\Users\moizp\Documents\projects\intel_project_final\models\best_120_epoch_paddle_model\model.pdparams"
    )
