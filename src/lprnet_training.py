...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img_dirs_train", default="./data/train", help="the training images path"
    )
    parser.add_argument(
        "--img_dirs_val", default="./data/validation", help="the validation images path"
    )
    parser.add_argument("--dropout_rate", default=0.5, help="dropout rate.")
    parser.add_argument(
        "--epoch", type=int, default=33, help="number of epoches for training"
    )
    parser.add_argument("--batch_size", default=128, type=int, help="batch size")
    parser.add_argument(
        "--stn_path",
        default=r"..models/license_plate_ocr_models/stn_Iter_043200_model.ckpt",
        help="path to stn checkpoint",
    )
    parser.add_argument("--checkpoint", default=r"..models/license_plate_ocr_models/lprnet_Iter_043200_model.ckpt", help="the path to checkpoint")
    ...
    STN.to(device)
    STN.load_state_dict(
        torch.load(args.stn_path, map_location=lambda storage, loc: storage)[
            "net_state_dict"
        ]
    )
    print("STN loaded")
    ...
    for epoch in range(args.epoch):
        ...
        for imgs, labels, lengths, lines in dataloader[
            "train"
        ]:  # img: torch.Size([2, 3, 24, 94])  # labels: torch.Size([14]) # lengths: [7, 7] (list)
            ...
            with torch.set_grad_enabled(True):
                ...
                if lines == "2":
                    transfer1 = (
                        transfer.cpu().detach().numpy().squeeze(0).transpose(1, 2, 0)
                    )
                    ...
                    new_transfer = (
                        torch.from_numpy(transposed).float().unsqueeze(0).to(device)
                    )
                ...
                if total_iters % 100 == 0:
                    ...
                    for i, length in enumerate(lengths):
                        ...
                        start += length
                        if np.array_equal(
                            np.array(pred_labels[i]), label.cpu().numpy()
                        ):
                            TP += 1
                    ...
                    for p in optimizer.param_groups:
                        lr = p["lr"]
                    print(
                        "Epoch {}/{}, Iters: {:0>6d}, loss: {:.4f}, train_accuracy: {:.4f}, time: {:.2f} s/iter, learning rate: {}".format(
                            epoch,
                            args.epoch - 1,
                            total_iters,
                            loss.item(),
            ...
            if total_iters % 900 == 0:
                torch.save(
                    {"iters": total_iters, "net_state_dict": lprnet.state_dict()},
                    os.path.join(save_dir, "lprnet_Iter_%06d_model.ckpt" % total_iters),
                )

                torch.save(
                    {"iters": total_iters, "net_state_dict": STN.state_dict()},
            ...
            if total_iters % 900 == 0:
                ...
                print(
                    "Epoch {}/{}, Iters: {:0>6d}, validation_accuracy: {:.4f}".format(
                        epoch, args.epoch - 1, total_iters, ACC
                    )
                )
