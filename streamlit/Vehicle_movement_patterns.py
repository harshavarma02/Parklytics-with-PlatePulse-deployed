...
st.sidebar.title("🚗 Vehicle Analysis System")
st.sidebar.markdown("---")

# Navigation menu
page_selection = st.sidebar.selectbox(
    "Navigate to:",
    ["🏠 Main Dashboard", "🔍 Detection Modules", "📊 Data Analysis"]
)

if page_selection == "🔍 Detection Modules":
    st.sidebar.markdown("### Detection Modules")
    st.sidebar.markdown("- License Plate Image")
    st.sidebar.markdown("- License Plate Video")
    st.sidebar.markdown("- Parking Lot Detection")
    st.info("Please use the pages in the sidebar to access Detection Modules")
    st.stop()

if page_selection == "📊 Data Analysis":
    st.sidebar.markdown("### Data Analysis")
    st.sidebar.markdown("- Vehicle Movement Patterns (Current Page)")
    st.sidebar.markdown("- Parking Occupancy Trends")
