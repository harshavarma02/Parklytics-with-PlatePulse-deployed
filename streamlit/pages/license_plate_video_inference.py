...
lprnet.load_state_dict(
    torch.load(
        "streamlit/weights/training_checkpoint_files/lprnet_Iter_015300_model.ckpt",
...
STN.load_state_dict(
    torch.load(
        "streamlit/weights/training_checkpoint_files/stn_Iter_015300_model.ckpt",
        map_location=lambda storage, loc: storage,
    )["net_state_dict"]
)
STN.eval()
...
def video_inference(video, video_path, model, frame_log_container, output_container):
    ...
    # For consecutive correct plate verification
    last_seen_plate = None
    consecutive_count = 0
    PLATE_STABILITY_THRESHOLD = 3
    confirmed_plates = []
    ...
    for result in model(video_path, stream=True):
        ...
        for bbox in bboxes.data:
            ...
            class_label = process_roi(roi, bbox_width, bbox_height).upper().replace(" ", "")
            ...
            # Check if it's same as last seen
            if class_label == last_seen_plate:
                consecutive_count += 1
            else:
                last_seen_plate = class_label
                consecutive_count = 1
            
            # If plate seen consistently for enough frames, accept it
            if consecutive_count >= PLATE_STABILITY_THRESHOLD and class_label not in confirmed_plates:
                confirmed_plates.append(class_label)
    ...
    if confirmed_plates:
        st.success(f"✅ {len(confirmed_plates)} unique vehicle(s) detected.")
        st.code("\n".join(confirmed_plates))
        
        # Optional CSV download
        df = pd.DataFrame(confirmed_plates, columns=["License Plate"])
        st.download_button("Download Plates", df.to_csv(index=False), "plates.csv")
    
    # Save the logs
    return count, sum_lpr_time / count if count > 0 else 0, sum_yolo_time / count if count > 0 else 0

# Streamlit app

st.title("🚗 License Plate Detection - Video Analysis")
st.markdown("""
### What this page does:
- **Detects license plates** in uploaded video files using YOLOv8 + LPRNet models
- **Filters out incorrect readings** by requiring plates to appear consistently across 3+ frames
- **Prevents duplicate counting** of the same vehicle
- **Provides real-time inference metrics** (processing speed)
- **Exports results** as downloadable CSV file
