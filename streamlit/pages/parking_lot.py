...
st.set_page_config(layout="wide", page_title="Parking Lot Detection")
...
Upload a video to monitor parking space occupancy using YOLOv8 binary classification.

**Features:**
- Real-time occupancy detection
- Visual indicators (green=empty, red=occupied)
...
if video is not None:
    ...
    try:
        ...
        st.markdown("---")
        st.subheader("📈 Parking Analysis Results")
        ...
        print(e)
        st.error(
            "⚠️ An error occurred during inference: No parking spaces detected. Please try again with a different video file."
        )
        st.session_state["video_loaded"] = False
else:
    st.info("📁 Please upload a video file to begin parking lot analysis.")
